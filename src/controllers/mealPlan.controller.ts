...
export class MealPlanController {
    ...
    async generateRemainingDays(
        ...
        try {
            ...
            const result =
                await MealPlanGeneratorUtil.generateRemainingWeekDays(
                    userId,
                    days
                );
            ...
            if (!result.success) {
                throw new BadRequestError(
                    result.error || "Failed to generate meal plan"
                );
            }

            new OkResponse(
                result.data,
                `${days} days generated successfully with NEW recipes`
            ).send(res);
        } catch (error) {
            next(error);
        }
