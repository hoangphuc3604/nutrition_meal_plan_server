...
class MealPlanGeneratorUtil {
  ...
  async generateRemainingWeekDays(
    ...
    try {
      console.log(`[INFO] - Starting ${numDays}-day generation for user: ${userId}`);
      ...
      console.log(`[INFO] - Week range: ${weekStartStr} to ${weekEndStr}`);
  ...
  async generateForSpecificWeek(
    ...
    try {
      console.log(`[INFO] - Generating ${numDays} days for week: ${weekStartDate}`);
